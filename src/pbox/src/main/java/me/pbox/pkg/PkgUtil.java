...
public class PkgUtil {
    ...
    public static File findPboxXmlFile(Pkg pkg) {
        for (String source : Sources.getList()) {
            ...
            File result = HttpUtil.getTemporaryFile(url, false, false);
            if (result != null) {
                return result;
            }
        }
    ...
    public static File findPbox7zFile(Pkg pkg) {
        for (String source : Sources.getList()) {
            ...
            File result = HttpUtil.getTemporaryFile(url, true, true);
            if (result != null) {
                return result;
            }
        }
    ...
    public static String findLatestVersion(Pkg pkg) {
        for (String source : Sources.getList()) {
            ...
            File pboxXmlFile = HttpUtil.getTemporaryFile(url, false, false);
            if (pboxXmlFile != null) {
                try {
